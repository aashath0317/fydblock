...
const ConnectApiOverlay = ({ navigate, title = "Connect" }) => (
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#050B0D]/60 backdrop-blur-[2px] transition-all rounded-2xl">
...
const BotCard = ({ bot, isConnected, navigate }) => {
    ...
    return (
        <div className={`bg-[#0A1014]/40 backdrop-blur-md rounded-2xl p-5 border border-white/5 min-w-[260px] relative overflow-hidden group transition-all ${isConnected ? 'hover:border-[#00FF9D]/30' : ''}`}>
            {!isConnected && <ConnectApiOverlay navigate={navigate} title="Connect" />}

            <div className={`${!isConnected ? 'filter blur-[2px] opacity-60 pointer-events-none' : ''} transition-all`}>
                <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span className="text-[10px] uppercase tracking-wider text-[#00FF9D]">{bot.bot_type}</span>
                        </div>
                    </div>
...
const Dashboard = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    let isConnected = data.botCreated;
                    ...
                    if (isConnected) {
                        ...
                        if (dashRes.ok) {
                            ...
                            // Check for valid bots
                            const validBots = dashData.bots.filter(b => !isInvalidBot(b.bot_type));
    ...
    return (
        <div className="flex h-screen bg-[#050B0D] font-sans text-white overflow-hidden selection:bg-[#00FF9D] selection:text-black relative">
            ...
            {/* --- ADDED: NEON BACKGROUND EFFECTS (Increased Glow) --- */}
                ...
                <div className="px-6">
                    ...
                    <nav className="space-y-1">
                        {[
                            ...
                            <button key={item.name} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group ${item.active ? 'bg-[#00FF9D] text-black shadow-[0_0_15px_rgba(0,255,157,0.3)]' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
            ...
            <main className="flex-1 overflow-y-auto p-4 md:p-8 relative z-10">
                ...
                <section className="grid md:grid-cols-3 gap-6 mb-8">
                    <StatCard title="Today's Profit" value={statsData.daily.value} percentage={statsData.daily.percentage} icon={<span className="font-bold text-lg">$</span>} isConnected={hasExchange} navigate={navigate} />
                    <StatCard title="30 Days Profit" value={statsData.monthly.value} percentage={statsData.monthly.percentage} icon={<span className="font-bold text-lg">30</span>} isConnected={hasExchange} navigate={navigate} />
                    <StatCard title="Assets Value" value={statsData.assets.value} percentage={statsData.assets.percentage} icon={<PieChart size={20} />} isConnected={hasExchange} navigate={navigate} />
                </section>

                {/* 2. PERFORMANCE ANALYTICS CHART */}
                <section className="bg-[#0A1014]/40 backdrop-blur-md rounded-3xl p-8 border border-white/5 mb-8 relative overflow-hidden min-h-[400px]">
                    {!hasExchange && <ConnectApiOverlay navigate={navigate} title="Connect API" />}
                        ...
                        <div className="flex justify-between items-center mb-8">
                            ...
                            <div className="flex bg-[#050B0D] p-1 rounded-lg border border-white/10">
                                {['1h', '3h', '1d', '1w', '1m'].map((tf, i) => (
                                    <button key={tf} className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${i === 2 ? 'bg-[#00FF9D] text-black' : 'text-gray-500 hover:text-white'}`}>{tf}</button>
                                ))}
                            </div>
                        </div>
                ...
                <section className="bg-[#0A1014]/40 backdrop-blur-md rounded-3xl p-8 border border-white/5 relative">
                    ...
                    <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide">
                        {validBots.map((bot, i) => <BotCard key={bot.id || i} bot={bot} isConnected={hasExchange} navigate={navigate} />)}
                        <div onClick={() => navigate('/bot-builder')} className="min-w-[200px] rounded-2xl border border-dashed border-white/20 flex flex-col items-center justify-center p-6 cursor-pointer hover:border-[#00FF9D] hover:bg-[#00FF9D]/5 transition-all group bg-[#0A1014]/40">
