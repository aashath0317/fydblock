    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#050B0D]/80 backdrop-blur-xl border-b border-white/10 py-4 shadow-lg transition-all duration-300">
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => handleNav('home')}>
          <img src="/logo.png" alt="FydBlock Logo" className="h-12 md:h-10 object-contain" />
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          <button
            onClick={() => handleNav('signin')} // <--- Update this to 'signin'
            className="text-white hover:text-[#00FF9D] font-medium transition-colors"
          >
            Log In
          </button>
          <button
            onClick={() => handleNav('signup')} // <--- Update to 'signup'
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-[#050B0D]/95 backdrop-blur-xl border-b border-white/10 p-6 flex flex-col gap-4 shadow-2xl animate-in slide-in-from-top-5">
          <button onClick={() => handleNav('home')} className="text-lg text-left text-gray-300">Home</button>
          <button onClick={() => handleNav('contact')} className="text-lg text-left text-[#00FF9D] font-bold">Contact Us</button>
          <div className="h-px bg-white/10 my-2"></div>
          <button className="bg-[#00FF9D] text-black py-3 rounded-lg font-bold text-center">Get Started</button>
        </div>
      )}
    </nav>
  );
