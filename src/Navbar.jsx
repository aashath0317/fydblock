...
const Navbar = () => {
  ...
  // Returns green if active, otherwise empty string (inherits default)
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#050B0D]/80 backdrop-blur-xl border-b border-white/10 py-4 shadow-lg transition-all duration-300">
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-300">
          {/* Note: In desktop, we append the active class. If not active, it stays gray-300 from parent div */}
          ...
          <Link to="/affiliate" className={`hover:text-[#00FF9D] transition-colors ${location.pathname === '/affiliate' ? 'text-[#00FF9D]' : ''}`}>Affiliate</Link>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-[#050B0D]/95 backdrop-blur-xl border-b border-white/10 p-6 flex flex-col gap-4 shadow-2xl h-screen">
          ...
          {/* FIXED: Applied dynamic styling to all mobile links */}
          <Link
            to="/"
          ...
          <Link
            to="/affiliate"
            onClick={() => setMobileMenuOpen(false)}
            className={`text-lg text-left hover:text-[#00FF9D] ${isActive('/affiliate')}`}
          >
            Affiliate
          </Link>
