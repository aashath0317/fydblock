        ...
        <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative z-10 lg:h-[600px] flex items-center justify-center">
            <div className="relative w-full max-w-[500px] animate-float">
              ...
              <img 
                src="/hero.png"
                onError={(e) => { e.currentTarget.src = "https://images.unsplash.com/photo-1620712943543-bcc4688e7485?q=80&w=1000&auto=format&fit=crop"; }}
                alt="AI Neural Network" 
                className="w-full h-auto object-contain relative z-10 drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-2xl border border-white/10"
              />
...
const WorldGlobe = () => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const connectionDist = globeRadius * 0.25;
      ...
      const projectedPoints = points.map(p => {
        ...
        return { x: x * scale + centerX, y: y * scale + centerY, z, alpha: Math.max(0.1, alpha) };
      });
        ...
        <div className="max-w-4xl mx-auto relative group">
          <div className="grid md:grid-cols-3 gap-6 mb-8">
             <div>
               ...
               <input type="text" value={asset} onChange={(e) => setAsset(e.target.value.toUpperCase())} className="w-full bg-[#0A1014] border border-white/10 rounded-lg p-3 text-white focus:border-[#00FF9D] focus:outline-none transition-colors font-mono focus:shadow-[0_0_10px_rgba(0,255,157,0.1)]" placeholder="e.g. BTC" />
             </div>
             <div>
               <label className="block text-xs text-gray-500 uppercase font-bold mb-2">Risk Level</label>
               <select value={risk} onChange={(e) => setRisk(e.target.value)} className="w-full bg-[#0A1014] border border-white/10 rounded-lg p-3 text-white focus:border-[#00FF9D] focus:outline-none transition-colors appearance-none cursor-pointer focus:shadow-[0_0_10px_rgba(0,255,157,0.1)]">
             ...
             <div className="flex items-end">
               <button onClick={generateStrategy} disabled={loading || !asset} className="w-full bg-[#00FF9D] hover:bg-[#00cc7d] disabled:opacity-50 disabled:cursor-not-allowed text-black font-bold py-3 rounded-lg transition-all flex items-center justify-center gap-2 hover:shadow-[0_0_20px_rgba(0,255,157,0.3)]">
